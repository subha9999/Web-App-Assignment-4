...
if(isset($_GET['searchCity'])) {
    ...
    $city = 'Dhaka'; // Default city if no city is provided
}
...
$forecastUrl = "http://api.openweathermap.org/data/2.5/forecast?q=" . $city . "&units=metric&appid=" . $apiKey;
    ...
    // Initialize variables to keep track of the unique dates
    $dates = array();
    $forecastCount = 0;
...
<body>
    ...
    foreach ($forecastData->list as $day) {
        ...
        // Check if this date is not already added
        if (!in_array($date, $dates)) {
            $dates[] = $date;
            $tempMax = $day->main->temp_max;

            // Display the forecast for this day
            ...
            // Break the loop when you have displayed the forecast for 4 days
            if ($forecastCount >= 4) {
                break;
            }
        }
